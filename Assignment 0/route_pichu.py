...
# Discussed with Shashank Kumar about the logic
...
def moves(map, row, col):
    ...
    # return (row, col)  # Return only moves that are within the board and legal (i.e. go through open space ".")
...
#
def moveString(move, curr_move, move_string):
...
# Referred the algorithm of BFS from the russell Norwig Book
    ...
    while fringe:
        ...
        for move in moves(house_map, *curr_move):
            ...
            if house_map[move[0]][move[1]] == "@":
                # movestring=moveString(move,curr_move,move_string)
                return (curr_dist + 1, moveString(move, curr_move, move_string))
            else:
                # movestring=moveString(move,curr_move,move_string)
                fringe.append((move, curr_dist + 1, moveString(move, curr_move, move_string)))
